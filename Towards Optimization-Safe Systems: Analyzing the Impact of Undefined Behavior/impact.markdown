...
||Конструкция| Достаточное условие | Неопределённое поведение         |
|-----------|-----------|---------------------|----------------------------------|
|Язык       | p + x     | p\_∞ + x\_∞ [0, 2^n− 1] | переполнение указателя           |
|| *p        | p = NULL            | разыменование нулевого указателя |
|| x op\_s y  | x\_∞ op\_s y\_∞ [−2^{n−1} , 2^{n−1} − 1] | переполнение знакового целочисленного |
...
Рисунок 4: Оптимизация нестабильного кода в популярных компиляторах: gcc, clang, aCC, armcc, icc, msvc, open64, pathcc, suncc, TMS320C6000 от TI, Diab compiler от  Wind River, и XL C compiler от IBM. В данных примерах `p` - указатель, `x` - знаковое целое, а `x^+` - положительное знаковое целое. В каждой клетке *"On"* означает что данная версия компилятора оптимизирует проверку в тождественно ложную и избавляется от неё на уровне оптимизации *n*, в то время как "-" значит что компилятор вообще не избавляется от данной проверки.
...
Удаление нестабильного кода так как это делают компиляторы в §2 является правом
