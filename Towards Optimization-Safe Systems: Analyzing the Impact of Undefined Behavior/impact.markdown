...
В спецификациях многих языков программирования присутствует описение некоторых фрагментов кода, обладающих неопределённым поведением  [15: §2.3], что разрешает компилятору создавать команды, с произвольным поведением. Например в C "использование непортируемых или ошибчных программных конструкций" приводит к неопределённому поведению [24: §3.4.3].

Один из способов, используемых компиляторами при работе с неопределённым поведением - оптимизировать программы, предполагая что программа никогда не попадает в состояние неопределённого поведения. Последствия таких оптимизаций бывают особенно удивительными для многих программистов: код, работающий правильно при выключенных оптимизациях (т.е. при `-O0`) работает неправильно с высокими уровнями оптимизацияй (т.е. от `-O2`), т.к. компилятор считает участки кода мёртвыми и удаляет их. Мы называем такой код оптимизационно-неустойчивым, или кортоко нестабильным кодом. Если удалённый код использовался для проверок безопасности, соптимизированные системы будут уязвимы для атак.
...
Для того чтобы понять что такое нестабильный код, рассмотрим проверку на переполнение указателя на рис. 1, где `buf` - указатель, а `len` - целое положительное число. Программист хотел отловить случай когда `len` на столько велико, что `buf + len` переполняется и проходит первую проверку на рис. 1. Мы нашли подобные проверки в нескольких системах, включая браузер Chromium [7], ядро Linux [49] и интерпритатор Python [37].

Хотя эта проверка работает с прямой адресацией, она проваливается на сегментированных архитектурах [23: §6.3.2.3]. Поэтому в стандарте C закреплено что переполненный указатель неопределён [24: §6.5.6/p8], что позволяет gcc предполагать что указатель определён всегда для любой архитектуры. Исходя из этого предположения `buf + len` должны быть больше чем buf, и эта проверока на "переполнение" всегда будет давать ложный результат. Следовательно gcc удаляет проверку, делая систему уязвимой для атак [13].
...
Кто прав в этом споре? С точки зрения компилятора, программисты сделали ошибку в коде. Например рис. 2 очвидно содержит ошибку, и даже на рис. 1 возможен некорректный код при строгой интерпритации стандарта C. Тем не менее, эти ошибки довольно тонкие, и их понимание требует глубокого знания спецификации языка. Поэтому неудивительно что такие ошибки продолжают распространяться.
...
Проверка сдвига `!(1 << x)` призваная отловить большое значение `x`, взята из патча к файловой системе ext4 [31]. Она не выполняется из-за предположения о невозможности переполнения сдвигом.
...
XL C compiler от IBM’s). Для каждого примера нестабильного кода мы смотрели, превратит ли компилятор в проверку в тождественно ложную и если да, то находили наименьший уровень оптимизации -On на котором это происходит. Результаты показаны в таблице 4.

Далее дял изучения эволюции оптимизаций, мы будем использовать gcc и clang, т.к. история их изменений легко доступна. Для gcc мы выбрали следующие показательные версии, охватывающие более десятилетия:

* gcc 2.95.3, последний из 2.x, выущен в 2001;
* gcc 3.4.6, последний из 3.x, выущен в 2006;
* gcc 4.2.1, последняя GPLv2 версия, выпущена в 2007 и до сих пор широко используется в BSD системах;
* gcc 4.8.1, последняя версия, выущена в 2013.
...
В-четвёртых, оптимизации используют неопределённое поведение не только из самого языка, но также из библиотечных функций (например `abs` [18] и `realloc` [40]) т.к. со временем компиляторы стали понимать их.
...
Удаление нестабильного кода так как это делают компиляторы в §2 является правомерным по стандарту языка, и не является ошибкой компилятора [39: §3]. Но это непонятно программистам. Мы поставили перед собой цель выявлять такие участки кода и выдавать предупреждения по ним. Как мы увидим в §6.2, эти предупреждения часто бывают на участках кода, которые программисты хотят исправить, а не заглушить предупреждения компилятора. Эта цель требует точной модели понимания нестабильного кода для гарантий того что предупреждения будут выданы только для нестабильного кода, а не для очевидно мертвого, который может быть безопасно удалён. Этот раздел вводит модель для размышлений о нестабильном коде и инструментарий с двумя алгоритмами его определения.

### Нестабильный код

Для формализации непонимая программистами стандарта C, которое приводит к нестабильному коду, давайте определим C\* как диалект C который обладает хорошо определённой семантикой для участков кода, имеющих неопределённое поведение в C. Например C\* определён для прямой адресации, нулевого указателя и охватывающей семантике для целочисленной арифметики и арифметики указателей [38]. Участок кода *e* это строка или выражение в конкретном месте кода в программе *P*. Если компилятор преобразует участок *e* способом, который меняет поведение *P* в C\*,  но не в C, то такой участок называется нестабильным.

Пусть *P[e/e′]* - программа, полученная методом замены *e* на некоторый участок *e'* на той же позиции исходного кода. Когда компилятор имеет право заменять *P* на *P[e/e']*, что обозначается как *P -> P[e/e']*? В языке, чья спецификация не содержит непределённого поведения ответ будет прямым: это правомерно для каждого случая, т.к. *P* и *P[e/e']* выдают один и тот же результат. В спецификации языка с неопределённым поведением ответ более сложный; а именно он состоит в том, что это верно для каждого случая, в котором выполняются:
...
Например, на рис. 4 в соответствии со стандартом C компилятор имеет право заменить разумные проверки тождественно ложным выражением, вто время как гипотетический компилятор C\* не может сделать тоже самое. Поэтому эти проверки также являются нестабильным кодом.

## Подход для определения нестабильного кода

Приведённое выше определение показывает что есть нестабильный код, но не показывает способов поиска нестабильного кода, т.к. сложно рассуждать о поведении всей программы. Как агент для поиска изменений в поведении программы, Stack ищет код, который может быть преобразован некоторой оптимизацийе _O_ в C, но не в C\*. В частности для этого Stack использует двух-фазовую схему:
...
Если _O_ на второй фазе оптимизирует дополнительный код, мы предполагаем что причина, по которой _O_ гн сделал этого на первой фазе заключается  в том, что это изменило бы семантику программы в C\* и таким образом Stack заключает что этот код нестабилен.

Основанный на оптимизациях подход Stack'а в нахождении нестабильного кода упускает нестабильный код, котороый оптимизатор не может удалить на второй фазе из-за особенностей своей работы, даже если существует какой-то оптимизатор, который может. Этот подход также может выдавать ложные результаты если оптимизатор недостаточно агрессивен в удалении кода на первой фазе.  Таким образом одной из задач при проектировании Stack было соответствие с достаточно агрессивным оптимизатором для минимизации данной проблемы.

Для работы такого подхода Stack требует оптимизатор, который может выборочно делать предположения о наличии неопределённого поведения. Для построения такого оптимизатора мы формализовали понятие "делать предположение о наличае неопределённого поведения" в §3.2.1 вводя хорошо _определённое программное допущение_, которое содержит допущение для языка C что программист никогда не напишет программу, приводящую к неопредлённому поведению. Работая с оптимизатором, который может принимать на вход явные допущения , Stack  может включить (или выключить) оптимизации, основанные на неопределённом поведении, сопровождая (или нет) хорошо-определённую программу допущение для оптимизатора. Мы создали два агрессивных оптимизатора, которые следуют этому допущению: один, который удаляет недостижимый код (§3.2.2) и другой, который упрощает необязательные вычисления (§3.2.3).

### Хорошо определённое программное допущение

Мы формализовали значение предположения о поведении компилятора следубщим образом. Предположим, что программе подаётся на вход *x*. Дан участок кода _e_, пусть __R_e__(*x*) означает условия его достижимости, которые выполняются если _e_ будет выполнен при входных данных _x_; и пусть __U_e__(*x*) означает его неопределённое поведение, или коротко условие UB, которое показывает, явялется ли _e_ неопределённым поведением при входных данных *x* исходя из семантики C (см. рис. 3).
...
Интуитивно, в хорошо-определённой программе при разыменовании указателя `p`, он не должен быть нулевым. Другими словами, отрицание условия UB, `p != NULL` должно выполнятсся всякий раз при исполнеии _e_. Ниже мы обощили этот пример.

*Определение 2* (допущение о хорошо определённой программе). Участок кода _e_ хорошо определён на входе *x* если выполнение _e_ никогда не приводит к неопределённому поведению в _e_:

    Re(x) → ¬Ue(x). (1)

Более того, программа хорошо определена на входных данных если каждый участок программы хорошо определён на входных данных, обозначенных ∆:

    ∆(x) = /\_{e∈P} Re(x) → ¬Ue(x). (2)

### Удаление недостижимого кода

Первый алгоритм выявляет нестабильный код, который модет быть удалён (т.е. __P ~> P[e/∅]__, где _e_ - выражение). Например если для достижения участка кода требуется ввести программу в неопределённое поведение, то этот участок должен быть недостижим. Мы формализовали это ниже.

**Теорема 1** (Удаление). В хорошо определённой программе _P_, оптимизатор может удалить фрагмент кода _e_ если не существует входных данных *x*, при которых достигается _e_ и удовлетворяется допущение о хооршо определённой программе *∆(x)*:
...
Используя вышеприведённое определение легко составить алгоритм выявления нестабильностей, приводящих к удалению кода (см. рис. 5). Алгоритм сначала удалаяет нестибильные участки без предположения о хорошо определённой программе, а потом предупреждает об участках, ставших недостижимыми с этим предположением. Последние являются нестабильным кодом.
