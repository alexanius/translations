...
Проверка сдвига `!(1 << x)` призвана отловить большое значение `x` из патча к файловой системе ext4 [31]. Она не выполняется из-за предположения о невозможности переполнения сдвигом.
